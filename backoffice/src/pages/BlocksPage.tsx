...
import { api, type BlockedResultEntry, type EventMatch } from "@granfondo/api";
    ...
    >
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="col-span-3">
          ...
          <input
            placeholder="Why this result is blocked"
            value={form.note}
...
export default function BlocksPage() {
  const [blocks, setBlocks] = useState<BlockedResultEntry[] | null>(null);
  ...
  const filtered = blocks?.filter(
    (block) => !search || matchesSearch(block, search),
  );
  ...
  return (
    <div>
      ...
      {blocks === null && <LoadingState error={error} />}
            ...
            <tbody className="divide-y divide-gray-100">
              {visible.map((block) => (
                <tr key={block.id} className="hover:bg-gray-50">
                  ...
                  <td className="py-2 pr-4 font-mono">{block.bib}</td>
                  <td className="py-2 pr-4">
                    <a
                      href={`/athlete/${block.blockedAthleteId}`}
