...
export const eventDistances = sqliteTable(
  "event_distances",
  {
    id: text("id").notNull(),
  ...
  (t) => [primaryKey({ columns: [t.id, t.eventId] })],
);
...
export const results = sqliteTable(
  ...
  (t) => [
    ...
    index("idx_results_athlete")
      .on(t.athleteId)
      .where(sql`athlete_id != 0`),
    index("idx_results_filters").on(
      t.eventId,
      t.distanceId,
      t.gender,
      t.category,
...
export const resultLicences = sqliteTable(
  "result_licences",
  {
  ...
  (t) => [primaryKey({ columns: [t.resultId, t.licence] })],
);
...
export const participants = sqliteTable(
  ...
  (t) => [index("idx_participants_event").on(t.eventId)],
);
...
export const athletes = sqliteTable(
  ...
  (t) => [index("idx_athletes_name_lower").on(t.nameLower)],
);

export const athleteTeams = sqliteTable(
  "athlete_teams",
  {
  ...
  (t) => [primaryKey({ columns: [t.athleteId, t.teamId] })],
);

export const athleteCategories = sqliteTable(
  "athlete_categories",
  {
    athleteId: integer("athlete_id")
      .notNull()
  ...
  (t) => [primaryKey({ columns: [t.athleteId, t.year, t.category] })],
);

export const athleteResults = sqliteTable(
  "athlete_results",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    athleteId: integer("athlete_id")
  ...
  (t) => [
    ...
    uniqueIndex("uq_athlete_results_race")
      .on(t.athleteId, t.eventId, t.distance)
      .where(sql`athlete_id != 0`),
  ],
);
...
export const aggregateAthletes = sqliteTable(
  "aggregate_athletes",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    year: integer("year").notNull(),
  ...
  (t) => [
    ...
    uniqueIndex("uq_agg_athlete_slice")
      .on(t.year, t.distance, t.gender, t.athleteId)
      .where(sql`athlete_id != 0`),
  ],
);

export const teamRanking = sqliteTable(
  "team_ranking",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    year: integer("year").notNull(),
  ...
  (t) => [index("idx_team_slice").on(t.year, t.distance, t.rank)],
);

export const teamRaceResults = sqliteTable(
  "team_race_results",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    teamRankingId: integer("team_ranking_id")
      .notNull()
      .references(() => teamRanking.id, { onDelete: "cascade" }),
    eventId: integer("event_id").notNull(),
  ...
  (t) => [index("idx_trr_ranking").on(t.teamRankingId)],
);

export const teamRaceAthletes = sqliteTable(
  "team_race_athletes",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    teamRaceResultId: integer("team_race_result_id")
      .notNull()
      .references(() => teamRaceResults.id, { onDelete: "cascade" }),
    athleteId: integer("athlete_id").notNull().default(0),
    name: text("name").notNull().default(""),
    pos: integer("pos").notNull().default(0),
...
export const aggregateResults = sqliteTable(
  "aggregate_results",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    aggregateAthleteId: integer("aggregate_athlete_id")
      .notNull()
      .references(() => aggregateAthletes.id, { onDelete: "cascade" }),
    eventId: integer("event_id").notNull(),
    eventName: text("event_name").notNull().default(""),
    eventDate: text("event_date").notNull().default(""),
  ...
  (t) => [index("idx_ar_athlete").on(t.aggregateAthleteId)],
);
