...
function insertEvents(
  ...
  for (const e of data.events) {
    ...
    const distancesToStore =
      resultDists && resultDists.length > 0 ? resultDists : e.distances;
    ...
    for (const dist of evResults.distances) {
      for (const r of dist.results) {
        ...
        for (const lic of r.licences) {
          db.insert(schema.resultLicences)
            .values({
              resultId,
              licence: lic,
            })
...
function insertParticipants(
  ...
  for (const [eventId, athletes] of data.allParticipants) {
    for (const a of athletes) {
      const athleteId =
        data.participantAthleteIds?.get(`${eventId}:${a.name}:${a.team}`) ?? 0;
      db.insert(schema.participants)
        .values({ eventId, ...a, athleteId })
        .run();
    }
  }
}
  ...
  for (const entry of data.athletesIndex.values()) {
    ...
    for (const [year, cats] of Object.entries(entry.categories)) {
      for (const cat of cats) {
        db.insert(schema.athleteCategories)
          .values({
            athleteId: entry.id,
            year: Number(year),
            category: cat,
    ...
    for (const r of entry.results) {
      db.insert(schema.athleteResults)
        .values({
          athleteId: entry.id,
          eventId: r.eventId,
...
function insertLookups(
  ...
  for (const [key, athleteId] of Object.entries(data.nameToId)) {
    db.insert(schema.athleteLookup)
      .values({ key, athleteId })
      .onConflictDoUpdate({
        target: schema.athleteLookup.key,
        set: { athleteId },
      })
      .run();
  }
...
function buildTeamIds(data: AllScrapedData): Map<string, number> {
  ...
  const maxExisting =
    data.teamIdStore.size > 0 ? Math.max(...data.teamIdStore.values()) : 0;
...
function insertTeams(
  ...
  for (const [rawAlias, rawCanonical] of Object.entries(data.teamAliases)) {
    ...
    const canonicalKey = resolveAliasChain(
      normalizeTeam(rawCanonical),
      data.teamAliases,
    );
    ...
    if (!aliasByCanonical.has(canonicalKey)) {
      aliasByCanonical.set(canonicalKey, []);
    }
  ...
  for (const [canonicalKey, id] of teamIds) {
    ...
    db.insert(schema.teams)
      .values({ id, canonicalKey, aliasKeys: JSON.stringify(aliasKeys) })
      .onConflictDoUpdate({
        target: schema.teams.id,
        set: { canonicalKey, aliasKeys: JSON.stringify(aliasKeys) },
      })
      .run();
  }
}
  ...
  for (const [year, distances] of Object.entries(data.aggregateRanking)) {
    for (const [distance, genders] of Object.entries(distances)) {
      for (const [gender, athletes] of Object.entries(genders)) {
        for (const a of athletes) {
          const aggRun = db
            .insert(schema.aggregateAthletes)
            .values({
              year: Number(year),
              distance,
              gender,
          ...
          for (const r of a.results) {
            db.insert(schema.aggregateResults)
              .values({
      ...
      for (const t of teams) {
        ...
        for (const r of t.results) {
          ...
          for (const a of r.athletes) {
            db.insert(schema.teamRaceAthletes)
              .values({
...
function insertStats(
  ...
  db.insert(schema.stats)
    .values({ key: "stats_json", value: JSON.stringify(data.stats) })
    .onConflictDoUpdate({
...
function insertResultAssignments(
  db: ReturnType<typeof drizzle>,
  data: AllScrapedData,
): void {
...
function pruneGhostAthletes(sqlite: BetterSqlite3.Database): void {
  ...
  sqlite
    .prepare(`DELETE FROM athlete_teams      WHERE athlete_id IN (${ghosts})`)
    .run();
  sqlite
    .prepare(`DELETE FROM athlete_categories WHERE athlete_id IN (${ghosts})`)
    .run();
  sqlite
    .prepare(`DELETE FROM athlete_lookup     WHERE athlete_id IN (${ghosts})`)
    .run();
  sqlite
    .prepare(`DELETE FROM athletes           WHERE id         IN (${ghosts})`)
    .run();
  ...
  sqlite
    .prepare(
      "DELETE FROM athlete_lookup     WHERE athlete_id NOT IN (SELECT id FROM athletes)",
    )
    .run();
  sqlite
    .prepare(
      "DELETE FROM aggregate_athletes WHERE athlete_id NOT IN (SELECT id FROM athletes)",
    )
    .run();
  sqlite
    .prepare(
      "UPDATE results            SET athlete_id = 0 WHERE athlete_id != 0 AND athlete_id NOT IN (SELECT id FROM athletes)",
    )
    .run();
  sqlite
    .prepare(
      "UPDATE participants       SET athlete_id = 0 WHERE athlete_id != 0 AND athlete_id NOT IN (SELECT id FROM athletes)",
    )
    .run();
  sqlite
    .prepare(
      "UPDATE team_race_athletes SET athlete_id = 0 WHERE athlete_id != 0 AND athlete_id NOT IN (SELECT id FROM athletes)",
    )
    .run();
