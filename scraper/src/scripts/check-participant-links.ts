...
const participants = db
  .prepare("SELECT event_id, athlete_id FROM participants")
  .all() as Array<{ event_id: number; athlete_id: number | null }>;
...
console.log(
  `Linked to athlete  : ${linked} (${Math.round((linked / participants.length) * 100)}%)`,
);
console.log(`Unlinked           : ${participants.length - linked}\n`);
console.log("Per event:");
for (const [eid, s] of [...statsPerEvent.entries()].sort(
  (a, b) => a[0] - b[0],
)) {
  const pct = Math.round((s.linked / s.total) * 100);
  console.log(
    `  ${(events.get(eid) ?? eid).toString().slice(0, 44).padEnd(46)} ${s.linked}/${s.total} (${pct}%)`,
  );
}

db.close();
try {
  fs.unlinkSync("/tmp/granfondo_check.db");
} catch {}
