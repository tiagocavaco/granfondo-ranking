...
function checkGaps(
  table: string,
  col: string,
): { count: number; maxId: number; gaps: number } {
  const { cnt, max_id } = db
    .prepare(`SELECT COUNT(*) as cnt, MAX(${col}) as max_id FROM ${table}`)
    .get() as { cnt: number; max_id: number };
...
console.log(
  `Athletes: ${athletes.count.toLocaleString()} rows, max ID ${athletes.maxId?.toLocaleString()}, gaps: ${athletes.gaps}`,
);
console.log(
  `Teams:    ${teams.count.toLocaleString()} rows, max ID ${teams.maxId?.toLocaleString()}, gaps: ${teams.gaps}`,
);
...
if (mode === "snapshot") {
  fs.writeFileSync(
    snapshotPath,
    JSON.stringify({ athletes, teams, top3, teamTop3 }, null, 2),
  );
...
if (mode === "verify") {
  ...
  for (let i = 0; i < Math.max(pre.top3.length, top3.length); i++) {
    ...
    if (
      ...
      if (++rankMismatches <= 5) {
        console.error(
          `✗ Agg rank mismatch [${p?.year} ${p?.distance} ${p?.gender} rank${p?.rank}]: ` +
  ...
  if (rankMismatches === 0) {
    console.log(`✓ Aggregate top-3 unchanged across all slices`);
  } else if (rankMismatches > 5) {
    console.error(`  ... and ${rankMismatches - 5} more`);
  }
  ...
  for (let i = 0; i < Math.max(pre.teamTop3.length, teamTop3.length); i++) {
    ...
    if (
      ...
      if (++teamMismatches <= 5) {
        console.error(
          `✗ Team rank mismatch [${p?.year} ${p?.distance} rank${p?.rank}]: ` +
  ...
  if (teamMismatches === 0) {
    console.log(`✓ Team top-3 unchanged across all slices`);
  } else if (teamMismatches > 5) {
    console.error(`  ... and ${teamMismatches - 5} more`);
  }
...
for (const r of top3) {
  ...
  console.log(
    `    ${r.rank}. ${r.name} (id=${r.athlete_id}) — ${r.total_points} pts`,
  );
}
...
for (const r of teamTop3) {
  ...
  console.log(
    `    ${r.rank}. ${r.team} (tid=${r.team_id}) — ${r.total_points} pts`,
  );
}
