...
if (!fs.existsSync(candPath)) {
  console.error(
    "split-candidates.json not found — run npm run db:find-splits first",
  );
  process.exit(1);
}
...
const candidates: CandidateEntry[] = JSON.parse(
...
const existingAliases = sqlite
  .prepare("SELECT name, canonical_team, aliases_json FROM athlete_alias_rules")
...
for (const rule of existingAliases) {
  const aliases = JSON.parse(rule.aliases_json) as Array<{
    name: string;
    team: string;
  }>;
  for (const alias of aliases) {
    alreadyAliased.add(
      `${rule.name}|${rule.canonical_team}|||${alias.name}|${alias.team}`,
    );
  }
}
...
const existingAssignments = sqlite
  .prepare("SELECT event_id, bib FROM result_assignments")
  .all() as { event_id: number; bib: string }[];

const alreadyAssigned = new Set(
  existingAssignments.map((r) => `${r.event_id}|${r.bib}`),
);
...
for (const c of approved) {
  ...
  if (useAlias) {
    ...
    if (alreadyAliased.has(dedupeKey)) {
      console.log(
        `  · already aliased: "${c.absorb.name}" @ "${c.absorb.team}" → "${c.keep.name}"`,
      );
      aliasSkipped++;
      continue;
    }
    ...
    console.log(
      `  + alias: "${c.absorb.name}" @ "${c.absorb.team}" → "${c.keep.name}" @ "${c.keep.team}"`,
    );
    alreadyAliased.add(dedupeKey);
    aliasAdded++;
  } else {
    const rows = getAbsorbResults.all(c.absorb.id) as {
      event_id: number;
      bib: string;
    ...
    for (const row of rows) {
      ...
      if (keepOwned.has(`${row.event_id}|${row.distance_name}`)) {
        distSkipped++;
        continue;
      }
      ...
      if (normName(row.name) !== absorbNorm) {
        nameSkipped++;
        continue;
      }
    ...
    if (added > 0) {
      const notes = [
        distSkipped > 0
          ? `${distSkipped} skipped — keep already has that event/distance`
          : "",
        nameSkipped > 0
          ? `${nameSkipped} skipped — name mismatch (pipeline misattribution)`
          : "",
      ]
        .filter(Boolean)
        .join(", ");
      console.log(
        `  + assignments (${added}): "${c.absorb.name}" → athlete ${c.keep.id} "${c.keep.name}"${notes ? ` (${notes})` : ""}`,
      );
      assignAdded += added;
    } else {
...
if (aliasAdded) {
  summary.push(`${aliasAdded} alias rule(s)`);
}

if (assignAdded) {
  summary.push(`${assignAdded} result assignment(s)`);
}
