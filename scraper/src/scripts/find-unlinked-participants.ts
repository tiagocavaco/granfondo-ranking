...
for (const row of db.prepare("SELECT key FROM athlete_lookup").all() as Array<{
  key: string;
}>) {
  ...
  if (!nameToTeams.has(namePart)) {
    nameToTeams.set(namePart, new Set());
  }
...
const participants = db
  .prepare("SELECT name, team, athlete_id FROM participants")
  .all() as Array<{ name: string; team: string; athlete_id: number | null }>;
db.close();
try {
...
const pairCounts = new Map<
  string,
  { from: string; to: string; count: number; names: Set<string> }
>();
...
for (const p of participants) {
  ...
  if (p.athlete_id && p.athlete_id !== 0) {
    continue;
  } // already linked
  ...
  if (!existingTeams) {
    continue;
  } // name not in DB at all — new athlete

  if (existingTeams.has(teamKey)) {
    continue;
  } // team already in lookup
  ...
  if (bestSim < 0.6 || !bestTeam) {
    continue;
  } // too dissimilar — probably a different athlete with same name
...
console.log(
  `${sorted.length} potential team alias pairs (≥2 athletes affected):\n`,
);
