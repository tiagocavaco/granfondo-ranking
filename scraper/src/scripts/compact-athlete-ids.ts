...
function compactIds(
  ...
  const rows = db
    .prepare(`SELECT ${pkCol} FROM ${idTable} ORDER BY ${pkCol} ASC`)
  ...
  db.transaction(() => {
    db.exec(
      `CREATE TEMP TABLE _id_map (old_id INTEGER PRIMARY KEY, new_id INTEGER NOT NULL)`,
    );
    ...
    db.exec(
      `UPDATE ${idTable} SET ${pkCol} = -${pkCol} WHERE ${pkCol} IN (SELECT old_id FROM _id_map)`,
    );
    for (const { table, col } of fkTargets) {
      db.exec(
        `UPDATE ${table} SET ${col} = -${col} WHERE ${col} IN (SELECT old_id FROM _id_map)`,
      );
    }
 ...
 */
function rewriteAthleteLookupsForTeamRemap(
  db: Database.Database,
  ...
  db.transaction(() => {
    db.exec(
      `CREATE TEMP TABLE _tmap (old_id INTEGER PRIMARY KEY, new_id INTEGER NOT NULL)`,
    );
    const ins = db.prepare("INSERT INTO _tmap (old_id, new_id) VALUES (?, ?)");
    for (const [o, n] of teamMap) {
      ins.run(o, n);
    }
    ...
    db.exec(
      `DELETE FROM athlete_lookup WHERE key IN (SELECT old_key FROM _lookup_rewrite)`,
    );
    db.exec(`
...
function verifySnapshot(
  ...
  if (pre.agg.length !== post.agg.length) {
    console.error(
      `  aggregate_athletes: ${pre.agg.length} pre-rows vs ${post.agg.length} post-rows`,
    );
    ok = false;
  }
  ...
  if (aggMismatches > 5) {
    console.error(`  ... and ${aggMismatches - 5} more aggregate mismatches`);
  }

  // 2. Full team ranking
  if (pre.teams.length !== post.teams.length) {
    console.error(
      `  team_ranking: ${pre.teams.length} pre-rows vs ${post.teams.length} post-rows`,
    );
    ok = false;
  }
  ...
  if (teamMismatches > 5) {
    console.error(`  ... and ${teamMismatches - 5} more team mismatches`);
  }

  // 3. Per-race top-50 profile reachability
  if (pre.races.length !== post.races.length) {
    console.error(
      `  results top-50: ${pre.races.length} pre-rows vs ${post.races.length} post-rows`,
    );
    ok = false;
  }
  ...
  if (raceMismatches > 5) {
    console.error(`  ... and ${raceMismatches - 5} more race mismatches`);
  }
  ...
  const checkGaps = (label: string, table: string, col: string) => {
    const ids = (
      db_post
        .prepare(`SELECT ${col} FROM ${table} ORDER BY ${col}`)
        .all() as Record<string, number>[]
    ).map((r) => r[col]!);
    for (let i = 0; i < ids.length; i++) {
      if (ids[i] !== i + 1) {
        console.error(
          `  ${label}: gap found — expected ${i + 1}, found ${ids[i]} at position ${i}`,
        );
        ok = false;
        return;
      }
    ...
    console.log(
      `  ${label}: ${ids.length.toLocaleString()} rows, no gaps (1..${ids[ids.length - 1] ?? 0})`,
    );
  };
  ...
  const checks: Array<[string, string]> = [
    [
      "SELECT COUNT(*) as c FROM athlete_lookup WHERE athlete_id NOT IN (SELECT id FROM athletes)",
      "athlete_lookup.athlete_id orphans",
    ],
    [
      "SELECT COUNT(*) as c FROM athlete_results WHERE athlete_id NOT IN (SELECT id FROM athletes)",
      "athlete_results.athlete_id orphans",
    ],
    [
      "SELECT COUNT(*) as c FROM results WHERE athlete_id != 0 AND athlete_id NOT IN (SELECT id FROM athletes)",
      "results.athlete_id orphans",
    ],
    [
      "SELECT COUNT(*) as c FROM aggregate_athletes WHERE athlete_id NOT IN (SELECT id FROM athletes)",
      "aggregate_athletes.athlete_id orphans",
    ],
    [
      "SELECT COUNT(*) as c FROM athlete_teams WHERE athlete_id NOT IN (SELECT id FROM athletes)",
      "athlete_teams.athlete_id orphans",
    ],
    [
      "SELECT COUNT(*) as c FROM athlete_categories WHERE athlete_id NOT IN (SELECT id FROM athletes)",
      "athlete_categories.athlete_id orphans",
    ],
    [
      "SELECT COUNT(*) as c FROM participants WHERE athlete_id != 0 AND athlete_id NOT IN (SELECT id FROM athletes)",
    ...
    [
      "SELECT COUNT(*) as c FROM team_ranking WHERE team_id NOT IN (SELECT id FROM teams)",
      "team_ranking.team_id orphans",
    ],
  ];
  for (const [sql, label] of checks) {
    const { c } = db_post.prepare(sql).get() as { c: number };
    if (c > 0) {
      console.error(`  ${label}: ${c}`);
      ok = false;
    }
  }
...
// Current stats
const aStats = db
  .prepare("SELECT COUNT(*) as cnt, MAX(id) as max_id FROM athletes")
  .get() as {
  cnt: number;
  max_id: number;
};
const tStats = db
  .prepare("SELECT COUNT(*) as cnt, MAX(id) as max_id FROM teams")
  .get() as {
  cnt: number;
  max_id: number;
};
...
console.log(
  `Athletes: ${aStats.cnt.toLocaleString()} rows, max ID ${aStats.max_id.toLocaleString()}, ${aGaps.toLocaleString()} gaps`,
);
console.log(
  `Teams:    ${tStats.cnt.toLocaleString()} rows, max ID ${tStats.max_id.toLocaleString()}, ${tGaps.toLocaleString()} gaps`,
);
...
// the next full scrape.
{
  ...
  if (orphaned > 0) {
    db.prepare(
      "DELETE FROM athlete_teams WHERE athlete_id NOT IN (SELECT id FROM athletes)",
    ).run();
    console.log(
      `Pre-flight: removed ${orphaned.toLocaleString()} orphaned athlete_teams rows (stale from prior incomplete compaction)\n`,
    );
  }
  ...
  if (orphanedTeam > 0) {
    db.prepare(
      "DELETE FROM athlete_teams WHERE team_id NOT IN (SELECT id FROM teams)",
    ).run();
    console.log(
      `Pre-flight: removed ${orphanedTeam.toLocaleString()} orphaned athlete_teams (team_id) rows\n`,
    );
  }
}
...
console.log(
  `  aggregate_athletes: ${preSnapshot.agg.length} rows (all ranks, all slices)`,
);
console.log(
  `  team_ranking:       ${preSnapshot.teams.length} rows (all ranks, all slices)`,
);
...
if (tGaps > 0) {
  ...
  console.log(
    `  athlete_lookup keys with out-of-range team ID after rewrite: ${staleKeys}`,
  );
} else {
...
if (aGaps > 0) {
  console.log(
    `Athletes: 1..${aNew.toLocaleString()} (${athleteMap.size.toLocaleString()} remapped)`,
  );
}

if (tGaps > 0) {
  console.log(
    `Teams:    1..${tNew.toLocaleString()} (${teamMap.size.toLocaleString()} remapped)`,
  );
}
