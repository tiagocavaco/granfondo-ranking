...
for (const row of db.prepare("SELECT key FROM athlete_lookup").all() as {
  key: string;
}[]) {
  const parts = row.key.split("|");
  const teamKey = parts[1];
  if (teamKey && teamKey.length >= 3 && !teamKey.startsWith("solo:")) {
    allKeys.add(teamKey);
  }
}
...
for (const row of db
  .prepare("SELECT DISTINCT team FROM results WHERE length(team) >= 3")
  .all() as {
  team: string;
}[]) {
...
function emitPair(a: string, b: string, requireSharedTokens: boolean) {
  ...
  if (existingAliases.has(a) || existingAliases.has(b)) {
    return;
  }
