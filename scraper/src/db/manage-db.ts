...
import { loadAliasMap, validateAndFlattenAlias, rewriteLookupKeysForAlias } from "./alias-utils.js";
...
function openDb(): { sqlite: BetterSqlite3.Database; db: ReturnType<typeof drizzle> } {
...
function cmdList(): void {
  ...
  for (const r of aliases) {
    const aliases2 = JSON.parse(r.aliasesJson) as Array<{ name: string; team: string }>;
    console.log(`  [${r.id}] "${r.name}" @ "${r.canonicalTeam}"`);
    for (const a of aliases2) console.log(`       alias: "${a.name}" @ "${a.team}"`);
    if (r.note) console.log(`       note: ${r.note}`);
  }

  console.log(`\n── Result assignments (${assignments.length}) ──────────────────────`);
  for (const r of assignments) {
    console.log(`  [${r.id}] event ${r.eventId} bib ${r.bib} → athlete ${r.athleteId}`);
    if (r.note) console.log(`       note: ${r.note}`);
  }
...
function cmdAdd(args: Record<string, string>): void {
  ...
  if (target === "alias") {
    ...
    if (!name || !team || !aliasName || !aliasTeam) {
      console.error("Usage: add alias --name X --team Y --alias-name A --alias-team B [--note N]");
      process.exit(1);
    }
    ...
    if (!eventId || !bib || !athleteId) {
      console.error("Usage: add assignment --event-id E --bib B --athlete-id A [--note N]");
      process.exit(1);
    }
    ...
    console.log(`✓ Added assignment: event ${eventId} bib ${bib} → athlete ${athleteId}`);
    ...
    if (aliasTeamRow && canonTeamRow) {
      ...
      if (changed > 0) console.log(`  · rewrote ${changed} athlete_lookup key(s): team ID ${aliasTeamRow.id} → ${canonTeamRow.id}`);
    }
...
function cmdRemove(args: Record<string, string>): void {
  ...
  if (target === "alias") {
    const { name } = args;
    if (!name) { console.error("Usage: remove alias --name X"); process.exit(1); }
    const result = db.delete(schema.athleteAliasRules)
      .where(eq(schema.athleteAliasRules.name, name)).run() as unknown as { changes: number };
    console.log(result.changes ? `✓ Removed alias rule for "${name}"` : `⚠ No alias rule found for "${name}"`);

  } else if (target === "assignment") {
    const { eventId, bib } = args;
    if (!eventId || !bib) { console.error("Usage: remove assignment --event-id E --bib B"); process.exit(1); }
    const result = db.delete(schema.resultAssignments)
      .where(and(
        eq(schema.resultAssignments.eventId, Number(eventId)),
        eq(schema.resultAssignments.bib, bib),
      )).run() as unknown as { changes: number };
    console.log(result.changes ? `✓ Removed assignment: event ${eventId} bib ${bib}` : `⚠ No assignment found`);
