...
import { SCRAPED_EVENTS_PATH, DB_ENC_PATH, DATA_DIR, TMP_DB_PATH } from "../paths.js";

// ── Scraped-events index ──────────────────────────────────────────────────────

export function loadScrapedEvents(filePath = SCRAPED_EVENTS_PATH): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return JSON.parse(fs.readFileSync(filePath, "utf-8")) as Record<string, string>;
}

export function saveScrapedEvents(index: Record<string, string>, filePath = SCRAPED_EVENTS_PATH): void {
  fs.writeFileSync(filePath, JSON.stringify(index, null, 2), "utf-8");
}

// ── Database output ───────────────────────────────────────────────────────────

export async function writeEncryptedDatabase(data: AllScrapedData): Promise<void> {
