...
function addToTeamsAndCategories(
  entry: AthleteEntry,
...
function runPass1(ctx: PipelineCtx): void {
  ...
  const licenceToResults = new Map<
    string,
    Array<{ event: StoredEvent; dist: StoredDistanceResults; r: StoredResult }>
  >();
  ...
  const licenceToFilteredResults = new Map<
    string,
    Array<{ event: StoredEvent; dist: StoredDistanceResults; r: StoredResult }>
  >();
...
function runPass3b(ctx: PipelineCtx): void {
  ...
  function findShortCandidates(
    ...
    if (secondTokenMatches.length > 0) {
      return secondTokenMatches;
    }
  ...
  for (const members of byTeamId.values()) {
    ...
    for (const long of [...members]) {
      if (!index.has(long.key)) {
        continue;
      }

      if (long.tokens.length < 3) {
        continue;
      }
  ...
  if (count > 0) {
    console.log(
      `  [pass3b] ${count} full-name profile(s) merged into short-name entries`,
    );
  }
}
...
export function buildAthletesIndex(
  ...
  runPass8(ctx);
  console.log(
    `  [pass3-8] ${teamCount} new team profiles, ${soloCount} solo profiles (${ctx.soloFlags.length} collision flag(s))`,
  );
