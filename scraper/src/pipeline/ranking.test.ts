...
describe("buildAggregateRanking", () => {
  ...
  it("groups results by year and distance", () => {
    const events = [
      ...
      mkEvent(2, 2026, "2026-03-15"),
    ];
    const loader = (id: number) =>
      mkEventResults(
        id,
        id === 1 ? 2025 : 2026,
        `${id === 1 ? "2025" : "2026"}-03-15`,
        [
          {
            id: "1",
            name: "Granfondo",
  ...
  it("normalizes distance names (BIG DAY → Granfondo)", () => {
    ...
    const loader = () =>
      mkEventResults(1, 2025, "2025-03-15", [
        { id: "1", name: "BIG DAY", finisherCount: 1, results: [mkResult()] },
      ]);
    const ranking = buildAggregateRanking([event], loader);
  ...
  it("excludes DNF and DNS from ranking", () => {
    ...
    const ranking = buildAggregateRanking([event], loader);
    expect(
      ranking["2025"]!["Granfondo"]!["M"]!.every(
        (a) => a.name !== "Dnf Athlete",
      ),
    ).toBe(true);
  });
  ...
  it("treats same name but different teams as separate athletes in ranking", () => {
    ...
    const loader = (id: number) =>
      mkEventResults(id, 2025, id === 1 ? "2025-03-15" : "2025-04-20", [
        {
          id: "1",
          name: "Granfondo",
          finisherCount: 1,
    ...
    const ranking = buildAggregateRanking(
      events,
      loader,
      new Map(),
      new Map(),
    ...
    const anaSilvaEntries = ranking["2025"]!["Granfondo"]!["M"]!.filter(
      (a) => a.name.toLowerCase() === "ana silva",
    );
  ...
  it("looks up id from athleteIndex when provided", () => {
    ...
    const athleteIndex = new Map([
      ["ana silva|1", mkAthleteEntry(99, "ana silva")],
    ]);
    const ranking = buildAggregateRanking(
      [event],
      loader,
  ...
  it("awards equal points to tied athletes (same genderPos)", () => {
    ...
    const loader = () =>
      mkEventResults(1, 2026, "2026-03-15", [
        {
          id: "1",
          name: "Granfondo",
          finisherCount: 300,
  ...
  it("ranks athletes by totalPoints descending", () => {
    ...
    const athletes = buildAggregateRanking([event], loader)["2025"]![
      "Granfondo"
    ]!["M"]!;
...
describe("buildAggregateRanking — athleteId consolidation", () => {
  it("consolidates results for same athlete racing under different teams when athleteId is set", () => {
    ...
    const athleteIndex = new Map([
      [
        "jose borges|team alpha",
        mkAthleteEntry(1, "jose borges", ["team alpha"]),
      ],
    ]);
    const loader = (id: number) =>
    ...
    const joseEntries = ranking["2026"]!["Granfondo"]!["M"]!.filter(
      (a) => a.name.toLowerCase() === "jose borges",
    );
  ...
  it("without athleteId, different teams produce separate entries (baseline)", () => {
    ...
    expect(
      buildAggregateRanking(events, loader, new Map(), new Map(), teamIdStore)[
        "2026"
      ]!["Granfondo"]!["M"]!.filter(
        (a) => a.name.toLowerCase() === "jose borges",
      ).length,
  ...
  it("uses athleteId to resolve id even when name+team key is not in athleteIndex", () => {
    const athleteIndex = new Map([
      [
        "jose borges|canonical team",
        mkAthleteEntry(42, "jose borges", ["canonical team"]),
      ],
    ]);
    const loader = () =>
  ...
  it("normalizes distance aliases before grouping (BIG DAY and Clássica → Granfondo)", () => {
    ...
    expect(
      ranking["2026"]!["Granfondo"]!["M"]!.find((a) => a.id === 1)
        ?.eventsScored,
    ).toBe(2);
  });
});
...
describe("buildAggregateRanking — points cutoff", () => {
  it("gives 0 points to finishers outside top 50 — they don't appear in ranking", () => {
    ...
    const gf_m = buildAggregateRanking(
      [mkEvent(1, 2026, "2026-02-15")],
      loader,
    )["2026"]!["Granfondo"]!["M"]!;
  ...
  it("athlete finishing outside top 50 does not appear in ranking", () => {
    ...
    const loader = () =>
      mkEventResults(1, 2026, "2026-02-21", [
        { id: "1", name: "Mediofondo", finisherCount: 200, results },
      ]);
    expect(
      buildAggregateRanking([mkEvent(1, 2026, "2026-02-21")], loader)["2026"]![
        "Mediofondo"
      ]!["M"]!.find((a) => a.name.toLowerCase() === "tiago cavaco"),
    ).toBeUndefined();
  });
});
...
describe("buildTeamRanking", () => {
  function mkTeamResults(
    teamAthletes: Array<{ name: string; pos: number; team: string }>,
  ): StoredEventResults {
    return mkEventResults(1, 2025, "2025-03-15", [
      {
        id: "1",
        name: "Granfondo",
        finisherCount: teamAthletes.length,
        results: teamAthletes.map((a, i) =>
          mkResult({
            pos: a.pos,
            genderPos: i + 1,
            raceTimeSecs: a.pos * 100,
            name: a.name,
            team: a.team,
          }),
        ),
  ...
  it("requires at least 3 athletes per team for eligibility", () => {
    ...
    const teams = buildTeamRanking([mkEvent(1, 2025, "2025-03-15")], loader)[
      "2025"
    ]!["Granfondo"]!;
  ...
  it("awards 25 base points to team rank 1 with 25 eligible teams (coeff = 1.00)", () => {
    ...
    expect(
      buildTeamRanking([mkEvent(1, 2025, "2025-03-15")], () =>
        mkTeamResults(athletes),
      )["2025"]!["Granfondo"]![0]!.totalPoints,
    ).toBe(25);
  });
  ...
  it("accumulates points across multiple events", () => {
    ...
    expect(
      buildTeamRanking(events, loader)["2025"]!["Granfondo"]![0]!.eventsScored,
    ).toBe(2);
  });
  ...
  it("handles loader returning null", () => {
    expect(
      Object.keys(
        buildTeamRanking([mkEvent(1, 2025, "2025-03-15")], () => null),
      ).length,
    ).toBe(0);
  });
  ...
  it("includes athlete IDs in team race results when athleteIndex provided", () => {
    ...
    expect(
      ranking["2025"]!["Granfondo"]![0]!.results[0]!.athletes.map((a) => a.id),
    ).toEqual([10, 20, 30]);
  });
});
