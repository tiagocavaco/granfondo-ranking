    ...
    for (const dist of res.distances) {
      for (const r of dist.results) {
        if (!isSoloTeam(r.team)) {
          ...
          if (key && !teamIdStore.has(key)) {
            teamIdStore.set(key, nextId++);
          }
        }
      }
    }
...
function mkTeamEvent(
  ...
  const loader = () =>
    mkEventResults(eventId, year, date, [
      {
        id: "1",
        name: distance,
        finisherCount: 100,
...
describe("athleteKey", () => {
  ...
  it("returns nameLower|solo:category for 'Individual' team with category", () => {
    expect(athleteKey("ana silva", "Individual", store, "MASTERS B")).toBe(
      "ana silva|solo:masters-b",
    );
  });

  it("returns nameLower|solo:category for 'Indivídual' (accented) with category", () => {
    expect(athleteKey("ana silva", "Indivídual", store, "MASTERS A")).toBe(
      "ana silva|solo:masters-a",
    );
  });
...
describe("isSoloTeam", () => {
  ...
  it("returns true for 'Independente'", () =>
    expect(isSoloTeam("Independente")).toBe(true));
  it("returns false for a real team", () =>
...
describe("buildAthletesIndex — duplicate event safeguard", () => {
  it("same licence, same event, different categories → one result kept, no duplicate", () => {
    ...
    const loader = () =>
      mkEventResults(1, 2025, "2025-04-27", [
        {
          id: "1",
          name: "Mediofondo",
          finisherCount: 100,
  ...
  it("same licence, same event, ambiguous categories → one result kept", () => {
    const events = [
      mkEvent(1, 2025, "2025-02-01"),
      mkEvent(2, 2025, "2025-04-27"),
    ];
    const loader = (id: number) =>
      mkEventResults(id, 2025, id === 1 ? "2025-02-01" : "2025-04-27", [
        {
          id: "1",
          name: "Granfondo",
          finisherCount: 100,
  ...
  it("same licence, two different events → two results kept", () => {
    const events = [
      mkEvent(1, 2025, "2025-03-01"),
      mkEvent(2, 2025, "2025-04-01"),
    ];
    const loader = (id: number) =>
      mkEventResults(id, 2025, `2025-0${id}-01`, [
        {
          id: "1",
          name: "Granfondo",
          finisherCount: 100,
...
describe("buildAthletesIndex — year-category consistency sweep", () => {
  it("5× MASTERS C + 1× MASTERS B in same year → MASTERS B result re-homed to separate entry", () => {
    ...
    const events = [1, 2, 3, 4, 5, 6].map((id) =>
      mkEvent(id, 2025, `2025-0${id}-01`),
    );
    const loader = (id: number) =>
      mkEventResults(id, 2025, `2025-0${id}-01`, [
        {
          ...
          results: [
            mkResult({
              ...
              licences: ["12345678"],
              team: "Individual",
              category: id === 5 ? "MASTERS B" : "MASTERS C",
              genderPos: 10,
              athleteId: 0,
            }),
          ],
    ...
    const main = entries.find((e) =>
      e.results.every((r) => r.category === "MASTERS C"),
    );
      ...
      mkEventResults(id, 2025, `2025-0${id}-01`, [
        {
          ...
          results: [
            mkResult({
              ...
              licences: ["12345678"],
              team: "CC Lagos",
              category: id === 1 ? "MASTERS B" : "MASTERS C",
              genderPos: 10,
              athleteId: 0,
            }),
          ],
...
describe("buildAthletesIndex — Pass 5: same-year solo grouping", () => {
  it("same name + category + year across different events → single profile", () => {
    const events = [
      mkEvent(1, 2026, "2026-02-15"),
      mkEvent(2, 2026, "2026-03-22"),
    ];
    const loader = (id: number) =>
      mkEventResults(id, 2026, id === 1 ? "2026-02-15" : "2026-03-22", [
        {
          id: "1",
          name: "Granfondo",
          finisherCount: 100,
    ...
    const soloEntries = [...index.values()].filter(
      (e) => e.nameLower === "joao silva",
    );
  ...
  it("same name + different category → two separate profiles", () => {
      ...
      mkEventResults(id, 2026, id === 1 ? "2026-02-15" : "2026-03-22", [
        {
          ...
          results: [
            mkResult({
              ...
              name: "Joao Silva",
              team: "",
              category: id === 1 ? "MASTERS A" : "MASTERS B",
              genderPos: 5,
              athleteId: 0,
            }),
          ],
    ...
    expect(
      [...runPipeline(events, loader).index.values()].filter(
        (e) => e.nameLower === "joao silva",
      ).length,
    ).toBe(2);
  });
  ...
  it("collision same event + same distance, divergent percentiles → resolved by percentile", () => {
    const events = [
      mkEvent(1, 2026, "2026-02-15"),
      mkEvent(2, 2026, "2026-03-22"),
      mkEvent(3, 2026, "2026-04-12"),
    ];
    const loader = (id: number) =>
      mkEventResults(
        id,
        2026,
        id === 1 ? "2026-02-15" : id === 2 ? "2026-03-22" : "2026-04-12",
        [
          {
            id: "1",
            name: "Granfondo",
    ...
    const { index, soloFlags } = runPipeline(events, loader);
    expect(
      [...index.values()].filter((e) => e.nameLower === "joao silva").length,
    ).toBe(2);
    expect(
      [...index.values()]
        .filter((e) => e.nameLower === "joao silva")
        .find((e) => e.results.length === 3),
    ).toBeDefined();
  ...
  it("collision same event + different distances → resolved by distance", () => {
    ...
    const loader = (id: number) => {
      if (id === 1) {
        return mkEventResults(1, 2026, "2026-02-15", [
          {
            id: "1",
            name: "Granfondo",
            finisherCount: 100,
    ...
    const { index, soloFlags } = runPipeline(events, loader);
    expect(
      [...index.values()].filter((e) => e.nameLower === "ana costa").length,
    ).toBe(2);
  ...
  it("collision same event + different distances, no prior events → resolved by distance", () => {
    const loader = () =>
      mkEventResults(1, 2026, "2026-02-15", [
        {
          id: "1",
          name: "Granfondo",
          finisherCount: 100,
    ...
    const { index, soloFlags } = runPipeline(
      [mkEvent(1, 2026, "2026-02-15")],
      loader,
    );
    expect(
  ...
  it("unresolvable collision (no baseline) → both bib-keyed + flagged_manual", () => {
    ...
    expect(
      [...index.values()].filter((e) => e.nameLower === "pedro nunes").length,
    ).toBe(2);
      ...
      mkEventResults(id, 2026, id === 1 ? "2026-02-15" : "2026-03-22", [
        {
          ...
          results: [
            mkResult({
              ...
              name: "Joao Silva",
              team: "",
              category: id === 1 ? "M Elite" : "MASTERS A",
              genderPos: 5,
              athleteId: 0,
            }),
          ],
...
describe("isValidCatTransition", () => {
  it("same category is always valid", () => {
    expect(isValidCatTransition("Masters B Male", "Masters B Male", 1)).toBe(
      true,
    );
  ...
  it("adjacent step (rank diff 1) is valid with 1+ year gap", () => {
    ...
    expect(isValidCatTransition("Masters B Male", "Masters C Male", 1)).toBe(
      true,
    );
  });
  ...
  it("going backwards in rank is never valid", () => {
    ...
    expect(isValidCatTransition("Masters A Male", "Elite Female", 10)).toBe(
      false,
    );
  });
  ...
  it("skipping a category (rank diff 2) requires at least 11 years", () => {
    ...
    expect(isValidCatTransition("Elite Male", "Masters B Male", 10)).toBe(
      false,
    );
    expect(isValidCatTransition("Elite Male", "Masters B Male", 11)).toBe(true);
    expect(isValidCatTransition("Masters A Male", "Masters C Male", 11)).toBe(
      true,
    );
    expect(isValidCatTransition("Masters A Male", "Masters C Male", 10)).toBe(
      false,
    );
  });

  it("skipping two categories (rank diff 3) requires at least 21 years", () => {
    expect(isValidCatTransition("Elite Male", "Masters C Male", 20)).toBe(
      false,
    );
...
describe("buildAthletesIndex — Pass 6: cross-year solo merge", () => {
  it("Masters A in 2025 + Masters B in 2026 → merged (valid step-up)", () => {
    ...
    const entries = [...runPipeline(events, loader).index.values()].filter(
      (e) => e.nameLower === "rui ferreira",
    );
    expect(entries.length).toBe(1);
    expect(entries[0]!.results.length).toBe(2);
  });
        ...
        [
          {
            ...
            results: [
              mkResult({
                ...
                name: "Rui Ferreira",
                team: "",
                category: id === 1 ? "MASTERS B" : "MASTERS A",
                genderPos: 5,
                athleteId: 0,
              }),
            ],
        ...
        [
          {
            ...
            results: [
              mkResult({
                ...
                name: "Rui Ferreira",
                team: "",
                category: id === 1 ? "MASTERS A" : "MASTERS C",
                genderPos: 5,
                athleteId: 0,
              }),
            ],
        ...
        [
          {
            ...
            results: [
              mkResult({
                ...
                name: "Carlos Matos",
                team: "",
                category: id === 1 ? "M 19-34" : "MASTERS A",
                genderPos: 5,
                athleteId: 0,
              }),
            ],
        ...
        [
          {
            ...
            results: [
              mkResult({
                ...
                name: "Carlos Matos",
                team: "",
                category: id === 1 ? "MASTERS A" : "M 19-34",
                genderPos: 5,
                athleteId: 0,
              }),
            ],
        ...
        [
          {
            ...
            results: [
              mkResult({
                ...
                name: "Carlos Matos",
                team: "",
                category: id === 1 ? "M 19-34" : "MASTERS B",
                genderPos: 5,
                athleteId: 0,
              }),
            ],
...
describe("buildAthletesIndex — Pass 7: cross-year team-change merge", () => {
  it("same name, 2 team profiles non-overlapping years, compatible category → merged", () => {
    const { event: e1, loader: l1 } = mkTeamEvent(
      1,
      2025,
      "2025-03-15",
      "Abel Carmona",
    ...
    const { event: e2, loader: l2 } = mkTeamEvent(
      2,
      2026,
      "2026-03-22",
      "Abel Carmona",
    ...
    ].filter((e) => e.nameLower === "abel carmona");
    expect(entries.length).toBe(1);
    expect(entries[0]!.results.length).toBe(2);
  });
  ...
  it("same name, 2 team profiles overlapping years → NOT merged (likely different people)", () => {
    ...
    expect(
      ...
      ].filter((e) => e.nameLower === "abel carmona").length,
    ).toBe(2);
  });
  ...
  it("valid pair not blocked by third same-name profile with year overlap (pairwise regression)", () => {
    ...
    const { event: e3, loader: l3 } = mkTeamEvent(
      3,
      2026,
      "2026-03-15",
      "Miguel Garcia",
    ...
    ].filter((e) => e.nameLower === "miguel garcia");
  ...
  it("non-overlapping years but divergent percentiles → NOT merged", () => {
    ...
    const events = [
      mkEvent(1, 2025, "2025-03-15"),
      mkEvent(2, 2025, "2025-06-15"),
      mkEvent(3, 2026, "2026-03-15"),
      mkEvent(4, 2026, "2026-06-15"),
    ];
    const loader = (id: number) =>
      mkEventResults(
        id,
  ...
  it("licence-only solo athlete (teamId=0, all-solo results) is NOT merged into a same-name team athlete", () => {
    ...
    const l2 = () =>
      mkEventResults(2, 2026, "2026-03-22", [
        {
          id: "1",
          name: "Granfondo",
          finisherCount: 100,
    ...
    const teamAthlete = entries.find((e) =>
      e.results.some((r) => r.team === "Team Carpintaria"),
    );
...
describe("buildAthletesIndex — Pass 8: team ↔ solo cross-pass merge", () => {
  it("solo result + team profile, no shared event, compatible → merged", () => {
    ...
    const { event: e2, loader: l2 } = mkSoloEvent(
      2,
      2025,
      "2025-06-15",
      "Maria Sousa",
    ...
    ].filter((e) => e.nameLower === "maria sousa");
    expect(entries.length).toBe(1);
    expect(entries[0]!.results.length).toBe(2);
  });

  it("solo + team sharing same eventId → NOT merged (golden rule)", () => {
    const { event: e1 } = mkTeamEvent(
      1,
      2025,
      "2025-03-15",
      "Maria Sousa",
    ...
    const loader = (id: number) =>
      mkEventResults(id, 2025, "2025-03-15", [
        {
          id: "1",
          name: "Granfondo",
          finisherCount: 100,
    ...
    expect(
      [...runPipeline([e1], loader).index.values()].filter(
        (e) => e.nameLower === "maria sousa",
      ).length,
    ).toBe(2);
  });
  ...
  it("solo Granfondo-only + team Minifondo-only → NOT merged (distance mismatch)", () => {
    ...
    expect(
      ...
      ].filter((e) => e.nameLower === "maria sousa").length,
    ).toBe(2);
  });

  it("solo top-5% + team bottom-60% (≥2 results each) → NOT merged (percentile mismatch)", () => {
    const events = [
      mkEvent(1, 2025, "2025-01-15"),
      mkEvent(2, 2025, "2025-02-15"),
      mkEvent(3, 2025, "2025-04-15"),
      mkEvent(4, 2025, "2025-05-15"),
    ];
    const loader = (id: number) =>
      mkEventResults(id, 2025, `2025-0${id}-15`, [
        {
          id: "1",
          name: "Granfondo",
          finisherCount: 100,
  ...
  it("2 team candidates survive all filters → flagged (no merge)", () => {
    const events = [
      mkEvent(1, 2025, "2025-02-01"),
      mkEvent(2, 2025, "2025-03-01"),
      mkEvent(3, 2025, "2025-04-01"),
    ];
    const loader = (id: number) =>
      mkEventResults(id, 2025, `2025-0${id}-01`, [
        {
          ...
          results: [
            mkResult({
              bib: String(id),
              name: "Maria Sousa",
              team: id === 1 ? "CC Faro" : id === 2 ? "Bike Team X" : "",
              category: "MASTERS A",
              genderPos: 5,
              athleteId: 0,
            }),
  ...
  it("solo with 1 result (inconclusive percentile) + 1 compatible team candidate → merged", () => {
    ...
    ].filter((e) => e.nameLower === "tiago neto");
    expect(entries.length).toBe(1);
    expect(entries[0]!.results.length).toBe(2);
  });
});
...
describe("buildAthletesIndex — athlete-aliases regression (Pass 7/8)", () => {
  it("Matteo Cigala — solo races merged into team profile by Pass 8", () => {
    ...
    const { event: e3, loader: l3 } = mkSoloEvent(
      3,
      2025,
      "2025-05-01",
      "Matteo Cigala",
    ...
    const { event: e4, loader: l4 } = mkSoloEvent(
      4,
      2025,
      "2025-06-01",
      "Matteo Cigala",
    ...
    ].filter((e) => e.nameLower === "matteo cigala");
    expect(entries.length).toBe(1);
  ...
  it("Gonçalo Freitas — solo race merged into team profile by Pass 8", () => {
    ...
    ].filter((e) => e.nameLower === "goncalo freitas");
    expect(entries.length).toBe(1);
    expect(entries[0]!.results.length).toBe(2);
  });
  ...
  it("Helder Loureiro — solo race merged into team profile by Pass 8", () => {
    ...
    ].filter((e) => e.nameLower === "helder loureiro");
    expect(entries.length).toBe(1);
    expect(entries[0]!.results.length).toBe(2);
  });
...
describe("buildAthletesIndex — Pass 9: manual result assignments", () => {
  it("manually assigned result exempt from post-pass category sweep", () => {
    const events = [1, 2, 3, 4, 5, 6].map((id) =>
      mkEvent(id, 2026, `2026-0${id}-01`),
    );
    const idStore: AthleteIdStore = new Map([
      ["carlos matos|solo:Masters B Male:2026", 100],
    ]);
    const loader = (id: number) =>
      mkEventResults(id, 2026, `2026-0${id}-01`, [
        {
          id: "1",
          name: "Granfondo",
          finisherCount: 100,
    ...
    const assignments: ResultAssignment[] = [
      { athleteId: 100, eventId: 6, bib: "6" },
    ];
    const { index } = buildAthletesIndex(
      events,
      loader,
      [],
      assignments,
  ...
  it("en-dash category variants (Coimbra 2024 format) are not dropped as outliers", () => {
    ...
    const loader = (id: number) =>
      mkEventResults(id, 2024, `2024-0${id}-01`, [
        {
          id: "1",
          name: "Granfondo",
          finisherCount: 200,
...
describe("buildAthletesIndex — Pass 1: licence majority-vote outlier detection", () => {
  ...
  it("non-dominant split (2 vs 2) is skipped entirely — licence not used", () => {
      ...
      mkEventResults(id, 2025, `2025-0${id}-01`, [
        {
          ...
          results: [
            mkResult({
              bib: String(id),
              name: id <= 2 ? "Rui Matos" : "Rui Oliveira",
              team: "Team Delta",
              licences: ["99999"],
              genderPos: id * 5,
              athleteId: 0,
    ...
    const ruis = [...index.values()].filter((e) =>
      e.nameLower.startsWith("rui "),
    );
...
describe("buildAthletesIndex — Pass 1: bib collision guard", () => {
  it("co-occurring licences with different bibs at the same event are NOT merged", () => {
    ...
    const lA = () =>
      mkEventResults(1, 2025, "2025-03-15", [
        {
          id: "1",
          name: "Granfondo",
          finisherCount: 100,
    ...
    const lB = () =>
      mkEventResults(2, 2025, "2025-06-15", [
        {
          id: "1",
          name: "Granfondo",
          finisherCount: 100,
    ...
    expect(entries.length).toBe(2);
    expect(entries.some((e) => e.results.some((r) => r.bib === "10"))).toBe(
      true,
    );
    expect(entries.some((e) => e.results.some((r) => r.bib === "20"))).toBe(
      true,
    );
  });
});
...
describe("buildAthletesIndex — post-pass backward transition override", () => {
  it("backward year sequence overrides earlier year's canon and evicts its results", () => {
    ...
    const events = [1, 2, 3, 4, 5, 6].map((id) =>
      mkEvent(
        id,
        id <= 3 ? 2024 : 2025,
        `202${id <= 3 ? 4 : 5}-0${id <= 3 ? id : id - 3}-01`,
      ),
    );
    const loader = (id: number) => {
  ...
  it("equal counts in a backward year do not block eviction when year is overridden", () => {
    ...
    const { index } = runPipeline(events, loader);
    const entries = [...index.values()].filter(
      (e) => e.nameLower === "egas vieira",
    );
    const main = entries.find((e) =>
      e.results.some((r) => r.category === "MASTERS A"),
    );
    expect(main).toBeDefined();
    expect(main!.results.filter((r) => r.category === "MASTERS D").length).toBe(
      0,
    );
  });
});
...
describe("buildAthletesIndex — post-pass non-adjacent forward-too-fast", () => {
  it("A→B→C over 3 years is valid (each step +1 rank, +1 year)", () => {
    ...
    const events = [1, 2, 3, 4, 5, 6, 7, 8, 9].map((id) =>
      mkEvent(
        id,
        id <= 3 ? 2024 : id <= 6 ? 2025 : 2026,
        `${id <= 3 ? 2024 : id <= 6 ? 2025 : 2026}-0${((id - 1) % 3) + 1}-01`,
      ),
    );
    const loader = (id: number) => {
      ...
      const cat =
        year === 2024 ? "MASTERS A" : year === 2025 ? "MASTERS B" : "MASTERS C";
    ...
    const { index } = runPipeline(events, loader);
    const entries = [...index.values()].filter(
      (e) => e.nameLower === "fabio monteiro",
    );
    const main = entries.find((e) =>
      e.results.some((r) => r.category === "MASTERS A"),
    );
    expect(main).toBeDefined();
    expect(main!.results.filter((r) => r.category === "MASTERS C").length).toBe(
      0,
    );
    expect(
      main!.results.filter(
        (r) => r.category === "MASTERS A" || r.category === "MASTERS B",
      ).length,
    ).toBe(6);
  });
  ...
  it("A→B→C over 22+ years is valid (enough time to reach C from A)", () => {
    ...
    const events = [
      mkEvent(1, 2004, "2004-03-01"),
      mkEvent(2, 2015, "2015-03-01"),
      mkEvent(3, 2026, "2026-03-01"),
    ];
    const loader = (id: number) => {
...
describe("isPortugueseNameAbbrev", () => {
  it("matches first + last token (core case)", () =>
    expect(
      isPortugueseNameAbbrev(["joao", "silva"], ["joao", "da", "silva"]),
    ).toBe(true));
  ...
  it("returns false when first tokens differ", () =>
    expect(
      isPortugueseNameAbbrev(["pedro", "silva"], ["joao", "da", "silva"]),
    ).toBe(false));

  it("returns false when last tokens differ", () =>
    expect(
      isPortugueseNameAbbrev(["joao", "costa"], ["joao", "da", "silva"]),
    ).toBe(false));

  it("returns false when short is not strictly shorter", () =>
    expect(isPortugueseNameAbbrev(["joao", "silva"], ["joao", "silva"])).toBe(
      false,
    ));

  it("returns false when short is longer than long", () =>
    expect(
      isPortugueseNameAbbrev(["joao", "da", "silva"], ["joao", "silva"]),
    ).toBe(false));
});
...
describe("isSpanishNameAbbrev", () => {
  it("matches first + second token for 3-token long name (core case)", () =>
    expect(
      isSpanishNameAbbrev(["luis", "garcia"], ["luis", "garcia", "fernandez"]),
    ).toBe(true));
  ...
  it("returns false when long name has more than 3 tokens", () =>
    expect(
      isSpanishNameAbbrev(
        ["luis", "garcia"],
        ["luis", "garcia", "fernandez", "lopez"],
      ),
    ).toBe(false));

  it("returns false when long name has fewer than 3 tokens", () =>
    expect(isSpanishNameAbbrev(["luis", "garcia"], ["luis", "garcia"])).toBe(
      false,
    ));

  it("returns false when first tokens differ", () =>
    expect(
      isSpanishNameAbbrev(["pedro", "garcia"], ["luis", "garcia", "fernandez"]),
    ).toBe(false));
...
describe("nameIsShortFormOf", () => {
  it("accepts Portuguese convention", () =>
    expect(nameIsShortFormOf(["joao", "silva"], ["joao", "da", "silva"])).toBe(
      true,
    ));

  it("accepts Spanish convention", () =>
    expect(
      nameIsShortFormOf(["luis", "garcia"], ["luis", "garcia", "fernandez"]),
    ).toBe(true));

  it("rejects when neither convention matches", () =>
    expect(nameIsShortFormOf(["joao", "costa"], ["joao", "da", "silva"])).toBe(
      false,
    ));

  it("rejects equal-length tokens", () =>
    expect(nameIsShortFormOf(["joao", "silva"], ["joao", "silva"])).toBe(
      false,
    ));
});
...
describe("pipeline pass 3b (short↔long name merge, same team)", () => {
  it("merges full-name entry into short-name entry (Portuguese convention)", () => {
    ...
    const entries = [...index.values()].filter(
      (e) => e.nameLower === "joao silva" || e.nameLower === "joao da silva",
    );
    expect(entries.length).toBe(1);
  ...
  it("merges full-name entry into short-name entry (Spanish convention)", () => {
    ...
    const entries = [...index.values()].filter(
      (e) =>
        e.nameLower === "luis garcia" ||
        e.nameLower === "luis garcia fernandez",
    );
    expect(entries.length).toBe(1);
  ...
  it("does NOT merge when long name has Spanish AND Portuguese match (ambiguous)", () => {
    ...
    const entries = [...index.values()].filter((e) =>
      ["luis garcia fernandez", "luis garcia", "luis fernandez"].includes(
        e.nameLower,
      ),
    );
