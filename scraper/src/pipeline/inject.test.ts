...
function mkEntry(
  id: number,
  nameLower: string,
  results: AthleteEntry["results"],
): AthleteEntry {
  return { id, name: nameLower, nameLower, teams: [], categories: {}, results };
}

function mkRef(
  overrides: Partial<AthleteEntry["results"][number]> = {},
): AthleteEntry["results"][number] {
  return {
    eventId: 1,
    eventName: "Event 1",
...
describe("injectAthleteIds", () => {
  it("injects athleteId when scraped name matches canonical name exactly", () => {
    const entry = mkEntry(42, "filipe oliveira", [
      mkRef({ eventId: 1, pos: 1, team: "Team Alpha", distance: "Granfondo" }),
    ]);
    const allResults = new Map([
      [
        1,
        mkEventResults(1, [
          { nameLower: "filipe oliveira", pos: 1, team: "Team Alpha" },
        ]),
      ],
    ]);
  ...
  it("injects athleteId when scraped name is a longer variant of the canonical name", () => {
    ...
    const entry = mkEntry(523, "filipe oliveira", [
      mkRef({
        eventId: 1,
        pos: 5,
        team: "Team Penacova",
    ...
    const allResults = new Map([
      [
        ...
        mkEventResults(1, [
          {
            nameLower: "filipe andre da silva oliveira",
            pos: 5,
            team: "Team Penacova",
          },
        ]),
  ...
  it("tied positions: two athletes at the same pos each get their correct athleteId via name fallback", () => {
    ...
    const entryA = mkEntry(10, "diogo graca", [
      mkRef({ eventId: 1, pos: 4, team: "Team A", distance: "Granfondo" }),
    ]);
    const entryB = mkEntry(20, "ricardo silva", [
      mkRef({ eventId: 1, pos: 4, team: "Team B", distance: "Granfondo" }),
    ]);
    const allResults = new Map([
      [
        1,
  ...
  it("does not assign athleteId via name fallback to an unrelated result at the same event", () => {
    ...
    const entry = mkEntry(17523, "joao pereira", [
      mkRef({ eventId: 1, pos: 195, team: "", distance: "Granfondo" }),
    ]);
    const allResults = new Map([
      [
        1,
  ...
  it("returns the count of events with at least one updated row", () => {
    ...
    const allResults = new Map([
      [
        1,
        mkEventResults(1, [{ nameLower: "test athlete", pos: 1, team: "T" }]),
      ],
      [
        2,
        mkEventResults(2, [{ nameLower: "test athlete", pos: 1, team: "T" }]),
      ],
    ]);

    const count = injectAthleteIds(
      new Map([["test athlete", entry]]),
      allResults,
    );
