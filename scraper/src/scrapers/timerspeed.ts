...
const CAT: Record<string, string> = {
  "elt m": "Elite Male",      "elt f": "Elite Female",
  "mam":   "Masters A Male",  "maf":   "Masters A Female",
  "mbm":   "Masters B Male",  "mbf":   "Masters B Female",
  "mcm":   "Masters C Male",  "mcf":   "Masters C Female",
  "mdm":   "Masters D Male",
  "mem":   "Masters E Male",
  "ebk":   "E-Bike",
  "cdtm":  "Junior Male",     "cdtf":  "Junior Female",
  "junm":  "Junior Male",     "junf":  "Junior Female",
  "para":  "Paracycling",
};
...
type Engage = { name: string; category: string; team: string; gender: string; distance: string };

async function parseClax(url: string): Promise<{ engages: Map<string, Engage>; times: Map<string, string> }> {
  const res = await fetchWithRetry(url);
  if (!res.ok) throw new Error(`TimerSpeed HTTP ${res.status} for ${url}`);
...
function buildDistances(
  ...
  for (const [bib, time] of times) {
    ...
    if (!eng?.distance) continue;
    if (!byDist.has(eng.distance)) byDist.set(eng.distance, []);
