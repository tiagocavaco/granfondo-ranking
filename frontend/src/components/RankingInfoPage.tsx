...
const ATHLETE_COEFF_SAMPLES = [
  ...
  finishers: f,
  coeff: finisherCoefficient(f).toFixed(2),
}));

const TEAM_COEFF_SAMPLES = [5, 10, TEAM_COEFFICIENT_REFERENCE, 50, 100].map(
  (t) => ({
    teams: t,
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 text-xs text-slate-400 uppercase tracking-wider border-b border-slate-100">
                  <th className="px-4 py-2 text-left">
                    {isAthlete ? "Gender position" : "Team rank"}
                  </th>
              ...
              <tbody className="divide-y divide-slate-100">
                {(isAthlete ? ATHLETE_POINTS : TEAM_POINTS).map((row) => (
                  <tr
                    key={"pos" in row ? row.pos : row.rank}
                    className="hover:bg-slate-50/60"
                  >
                    <td className="px-4 py-2 text-slate-700 font-medium">
                      {"pos" in row ? row.pos : row.rank}
                    </td>
          ...
          <div className="rounded-2xl border border-slate-200 overflow-hidden bg-white">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 text-xs text-slate-400 uppercase tracking-wider border-b border-slate-100">
                  <th className="px-4 py-2 text-left">
                    {isAthlete ? "Finishers" : "Teams"}
                  </th>
                  <th className="px-4 py-2 text-right">Coefficient</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {(isAthlete ? ATHLETE_COEFF_SAMPLES : TEAM_COEFF_SAMPLES).map(
                  (row) => {
                    const val = "finishers" in row ? row.finishers : row.teams;
                    const isRef =
                      (isAthlete && val === ATHLETE_COEFFICIENT_REFERENCE) ||
                      (!isAthlete && val === TEAM_COEFFICIENT_REFERENCE);
                    return (
                      <tr
                        key={val}
                        className={
