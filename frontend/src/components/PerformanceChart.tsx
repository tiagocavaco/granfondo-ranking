...
const CANON: Record<string, string> = {
  ...
  "Clássica": "Granfondo", GranFondo: "Granfondo",
  "Etapa": "Mediofondo", "TIME TRIAL": "Time Trial",
};

function canonDist(d: string) { return CANON[d] ?? d; }
function distColor(d: string) { return DIST_COLOR[d] ?? "#64748b"; }
    ...
    return (
      <div className="bg-white border border-slate-200 rounded-xl shadow-lg px-3 py-2 text-xs">
        ...
        <div className="flex items-center gap-2">
          <span style={{ color: distColor(pt.dist) }} className="font-bold">●</span>
      ...
      <div className="flex gap-3 mb-3 flex-wrap">
        {uniqueDists.map((d) => (
          <span key={d} className="flex items-center gap-1 text-xs text-slate-500">
            <span className="w-2.5 h-2.5 rounded-full inline-block" style={{ background: distColor(d) }} />
            {d}
          </span>
        ))}
      </div>
        ...
        <LineChart data={flat} margin={{ top: 20, right: 8, left: -20, bottom: 0 }}>
          ...
          <Line
            ...
            dot={(props: any) => {
              ...
              const color = distColor(payload.dist);
              const label = pos <= 3
                ? (pos === 1 ? "🥇" : pos === 2 ? "🥈" : "🥉")
                : `#${pos}`;
            ...
            activeDot={(props: any) => {
              const color = distColor(props.payload?.dist);
