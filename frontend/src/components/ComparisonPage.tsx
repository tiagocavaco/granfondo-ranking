...
function AthleteSearch({
  label, color, excluded, onSelect, selectedName, autoFocus,
}: {
  label: string; color: string; excluded?: number; onSelect: (a: AthleteRow) => void; selectedName?: string; autoFocus?: boolean;
}) {
        ...
        <ul className="absolute z-20 mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-lg max-h-56 overflow-y-auto">
          {results.map((a) => (
            ...
            >
              ...
              {a.canonicalTeam && <span className="text-xs text-slate-400 truncate max-w-[120px]">{a.canonicalTeam}</span>}
            </li>
          ))}
        </ul>
      )}
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-lg px-3 py-2 text-xs">
        ...
        {payload.map((p: any, i: number) => (
          <div key={p.dataKey} className="flex items-center gap-2">
            ...
            <span className="text-slate-600">{i === 0 ? (aData?.name ?? "A") : (bData?.name ?? "B")}:</span>
  ...
  return (
    <div>
      ...
      {aData && bData && (
        <>
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            {[{ data: aData, color: COLORS[0], wins: aWins }, { data: bData, color: COLORS[1], wins: bWins }].map(({ data, color, wins }, i) => {
              const finished = data.results.filter((r) => !r.dnf && !r.dns);
              const bestPos = finished.length > 0 ? Math.min(...finished.map((r) => r.pos)) : null;
              return (
                <div key={i} className="rounded-2xl border-2 p-4" style={{ borderColor: color + "40", background: color + "08" }}>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-sm">{countryFlag(data.results[0]?.country ?? "")}</span>
                    <span
                  ...
                  {data.canonicalTeam && (
                    <p className="text-xs text-slate-400 mb-3 truncate">{data.canonicalTeam}</p>
                  )}
                  <div className="flex gap-3 text-center">
                    <div>
                      <div className="text-lg font-extrabold" style={{ color }}>{wins}</div>
                      <div className="text-[10px] text-slate-400 uppercase">Wins</div>
                    </div>
                    {bestPos && (
                      <div>
                        <div className="text-lg font-extrabold text-slate-700">#{bestPos}</div>
                        <div className="text-[10px] text-slate-400 uppercase">Best</div>
                      </div>
                    )}
                    <div>
                      <div className="text-lg font-extrabold text-slate-700">{finished.length}</div>
                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    <LineChart data={chartData} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                      ...
                      <Legend
                        ...
                        formatter={(_, __, i) => i === 0 ? aData.name : bData.name}
                      />
                      <Line type="monotone" dataKey="a" name="a" stroke={COLORS[0]} strokeWidth={2} dot={{ r: 4, fill: COLORS[0], strokeWidth: 0 }} activeDot={{ r: 6 }} connectNulls={false} />
                      <Line type="monotone" dataKey="b" name="b" stroke={COLORS[1]} strokeWidth={2} dot={{ r: 4, fill: COLORS[1], strokeWidth: 0 }} activeDot={{ r: 6 }} connectNulls={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              )}
      ...
      {aData && !bData && !bLoading && (
        <div className="text-center py-12 text-slate-400 text-sm">Select a second athlete to compare</div>
      )}
      {!aData && !aLoading && (
        <div className="text-center py-12 text-slate-400 text-sm">Search for two athletes above to get started</div>
      )}
    </div>
  );
}
