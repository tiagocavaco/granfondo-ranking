...
import { predictionDistCoeff, predictionYearCoeff } from "@granfondo/utils/distance";
...
function computeWeightedScore(
  ...
  for (const { distance, year, pts } of entries) {
    total += pts * predictionDistCoeff(registeredDist, distance) * predictionYearCoeff(year, currentYear);
  }
  return total;
}
...
export const api = {
  ...
  async getPredictions(eventId: number): Promise<Record<string, DistancePredictions>> {
    const db = await getDb();
    const currentYear = new Date().getFullYear();

    // Linked participants — no scoring join needed here; scores computed separately
    const linkedRows = db.select({
    ...
    if (linkedIds.length > 0) {
      const raceCountRows = db.select({
        ...
        .all();
      for (const r of raceCountRows) raceCountMap.set(r.athleteId, r.cnt);
    }
    ...
    if (linkedIds.length > 0) {
      const ptsByDistRows = db.select({
        ...
        .groupBy(schema.aggregateAthletes.athleteId, schema.aggregateAthletes.distance, schema.aggregateAthletes.year)
        .all();
      ...
      for (const r of ptsByDistRows) {
        ...
        ptsByAthlete.get(r.athleteId)!.push({ distance: r.distance, year: r.year, pts: r.pts });
    ...
    for (const r of newcomerRows) newcomerMap.set(`${r.distance}|${r.category}`, r.cnt);
    ...
    for (const row of linkedRows) {
      ...
      const weightedScore = computeWeightedScore(distance, ptsByAthlete.get(athleteId), currentYear);

      const pred: FavoritePrediction = { athleteId, name, distance, category, gender, team, country, weightedScore, raceCount, mainDistance };

      if (!result[distance]) result[distance] = { overallMale: null, overallFemale: null, categories: {} };
      ...
      if (!distPreds.categories[category]) {
        distPreds.categories[category] = { ranked: [], newcomers: newcomerMap.get(`${distance}|${category}`) ?? 0 };
      }
      if (weightedScore > 0) {
    ...
    for (const r of newcomerRows) {
      if (!result[r.distance]) result[r.distance] = { overallMale: null, overallFemale: null, categories: {} };
