...
test("results table Gap column is hidden below lg breakpoint", async ({ page }) => {
  ...
  const finishedLink = page
    ...
    .filter({ hasText: /finishers/i })
    .first();
  if ((await finishedLink.count()) === 0) { test.skip(); return; }
  await finishedLink.click();
  await page.waitForURL(/\/event\/\d+/);

  const gapHeader = page.getByRole("columnheader", { name: /^Gap$/i }).first();
  if ((await gapHeader.count()) === 0) { test.skip(); return; }
  const display = await gapHeader.evaluate((el) => getComputedStyle(el).display);
  expect(display).toBe("none");
});

test("results table Gap column is visible at lg+ (desktop)", async ({ page }, testInfo) => {
...
test("athlete career table Gap column hidden below lg breakpoint", async ({ page }) => {
  const vp = page.viewportSize()!;
  if (vp.width >= 1024) { test.skip(); return; }
  await page.goto("/athlete/22");
  await page.waitForSelector("h1", { timeout: 15000 });
  const gapHeaders = page.getByRole("columnheader", { name: /^Gap$/i });
  if ((await gapHeaders.count()) === 0) { test.skip(); return; }
  const display = await gapHeaders.first().evaluate(
...
test("main content is not obscured by header on mobile", async ({ page }, testInfo) => {
  ...
  if (!headerBox || !h1Box) { test.skip(); return; }
